...
(function () {
  ...
  // Whole unit is one link — clicking icon or label both open WhatsApp
  var wrapper = document.createElement('a');
  wrapper.id = 'wa-fab';
  wrapper.href = 'https://wa.me/6580743630?text=Hi!%20I%27d%20like%20to%20incorporate%20a%20company%20in%20Singapore.%20Can%20we%20have%20a%20quick%20chat%3F';
  wrapper.target = '_blank';
  wrapper.rel = 'noopener noreferrer';
  wrapper.setAttribute('aria-label', 'Chat on WhatsApp');
  wrapper.style.cssText = 'position:fixed;bottom:24px;right:24px;display:flex;flex-direction:column;align-items:center;gap:8px;z-index:1050;text-decoration:none;transition:transform .2s ease;';

  var icon = document.createElement('div');
  icon.style.cssText = 'width:56px;height:56px;background:#25D366;border-radius:50%;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(37,211,102,.45);transition:box-shadow .2s ease;';
  icon.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="30" height="30" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347zM12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413A11.815 11.815 0 0 0 12.05 0zm0 21.785h-.004a9.86 9.86 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.861 9.861 0 0 1-1.51-5.26c.002-5.45 4.436-9.884 9.889-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.892 6.993c-.002 5.451-4.437 9.885-9.885 9.885z"/></svg>';

  var label = document.createElement('span');
  label.textContent = 'Chat with a human';
  label.style.cssText = 'font-size:10px;font-weight:700;color:#fff;background:#25D366;border-radius:100px;padding:4px 11px;white-space:nowrap;letter-spacing:.03em;font-family:inherit;box-shadow:0 2px 8px rgba(37,211,102,.35);';

  wrapper.addEventListener('mouseenter', function () {
    wrapper.style.transform = 'scale(1.08)';
    icon.style.boxShadow = '0 6px 28px rgba(37,211,102,.65)';
  });
  wrapper.addEventListener('mouseleave', function () {
    wrapper.style.transform = 'scale(1)';
    icon.style.boxShadow = '0 4px 20px rgba(37,211,102,.45)';
  });
